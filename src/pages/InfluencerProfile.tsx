...
export default function InfluencerProfile() {
  ...
  if (!influencer) {
    return (
      <div className="flex flex-col items-center justify-center p-20 space-y-4">
        <h2 className="text-2xl font-black text-slate-400 uppercase tracking-widest">Creator Record Not Found</h2>
        <button onClick={() => navigate('/influencers')} className="btn-primary">Return to Roster</button>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-10 pb-20 animate-in fade-in slide-in-from-bottom-6 duration-500">
           ...
           <div className="flex-1 space-y-6 text-center md:text-left">
              <div className="flex flex-wrap items-center justify-center md:justify-start gap-4">
                 ...
                 <div className={cn(
                   ...
                   influencer.status === 'Confirmed' ? "bg-emerald-500/10 border-emerald-500/30 text-emerald-400" : "bg-white/10 border-white/20 text-slate-400"
                 )}>
              ...
              <div className="flex flex-wrap justify-center md:justify-start gap-4">
                 <button className="px-8 py-3.5 bg-white text-slate-900 rounded-2xl font-black uppercase text-xs tracking-widest hover:scale-105 transition-transform flex items-center gap-3 shadow-xl">
                    Deploy Invitation <Zap size={16} fill="currentColor" />
                 </button>
                 <button className="px-8 py-3.5 bg-slate-800 text-white border border-slate-700 rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-slate-700 transition-colors flex items-center gap-3">
                    Extract Data Source <ExternalLink size={16} />
                 </button>
                 <button 
                  onClick={() => navigate(-1)}
                  className="px-8 py-3.5 bg-transparent text-white border border-white/10 rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-white/5 transition-all">
                    Return to Roster
                 </button>
              </div>
           </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
         <div className="lg:col-span-8 grid grid-cols-1 md:grid-cols-2 gap-8">
            {PERFORMANCE_METRICS.map((metric, idx) => (
              <div key={idx} className="command-card p-10 bg-white border-2 border-slate-50 rounded-[2.5rem] group hover:border-[var(--gc-orange-soft)] transition-all flex flex-col justify-between overflow-hidden relative">
                 ...
                 <div>
                    <p className="data-label text-slate-400 mb-2">{metric.label}</p>
                    <p className="text-5xl font-display font-black text-slate-900 group-hover:scale-105 transition-transform origin-left">{metric.value}</p>
                 </div>
                 <div className="mt-8 flex items-center justify-between">
                    <div className="flex items-center gap-2 px-3 py-1 bg-slate-50 text-slate-500 rounded-lg text-[9px] font-black uppercase tracking-widest">
                       {metric.baseline}
                    </div>
                    <span className={cn(
         ...
         <div className="lg:col-span-4 command-card bg-slate-900 p-10 rounded-[3.5rem] text-white flex flex-col justify-between shadow-2xl space-y-10 border border-slate-800">
            <div className="space-y-6">
               <div className="flex items-center gap-3">
                  <h3 className="section-title text-base tracking-widest uppercase opacity-80">Audience Pulse</h3>
      ...
      <div className="command-card bg-white border-2 border-slate-50 rounded-[3rem] overflow-hidden shadow-[0_30px_70px_rgba(0,0,0,0.02)]">
         <div className="p-10 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
            <div>
               <h3 className="section-title text-base tracking-widest uppercase">Collaborative Portfolio</h3>
               <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">Verified campaigns in the GC ecosystem</p>
            </div>
            <button className="px-6 py-3 bg-slate-900 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-[var(--gc-orange)] transition-all flex items-center gap-2">
         ...
         <div className="grid grid-cols-1 md:grid-cols-3 divide-x divide-slate-50">
            {[
              ...
              <div key={i} className="p-10 space-y-6 hover:bg-slate-50/50 transition-colors group cursor-pointer">
                 <div className="flex justify-between items-start">
                    <div className="w-12 h-12 bg-white border-2 border-slate-50 rounded-2xl flex items-center justify-center text-slate-400 group-hover:bg-slate-900 group-hover:text-white transition-all shadow-sm">
                       <Layers size={22} />
                    </div>
                    <span className={cn(
                 ...
                 <div className="space-y-1">
                    <h5 className="text-lg font-display font-black text-slate-900 group-hover:text-[var(--gc-orange)] transition-colors">{p.campaign}</h5>
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{p.vibe} • {p.date}</p>
                 </div>
                 <button className="w-full py-4 text-slate-400 group-hover:text-slate-900 border-t border-slate-50 group-hover:border-slate-200 transition-all text-[10px] font-black uppercase tracking-widest text-left pt-6 flex items-center justify-between">
