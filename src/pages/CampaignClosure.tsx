    ...
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-500">
      <div className="flex justify-between items-end mb-10">
        <div className="space-y-1">
          <div className="section-kicker">Stage 18 Lifecycle</div>
          <h2 className="section-title text-4xl">Campaign Closure</h2>
          <p className="text-[var(--ink-700)] text-[14px]">Final reconciliation and archive procedures.</p>
        </div>
        <div className="flex items-center gap-4">
          <select className="px-5 py-3 h-[46px] border border-[var(--border-strong)] rounded-xl text-[12px] bg-white font-bold text-[var(--ink-900)] outline-none focus:border-[var(--gc-purple)] focus:ring-[4px] focus:ring-[var(--gc-purple-soft)] transition-all shadow-sm">
            <option>{ClosureOutcome.COMPLETED}</option>
            <option>{ClosureOutcome.PARTIAL}</option>
            <option>{ClosureOutcome.CANCELLED}</option>
          </select>
          <button className="btn-primary px-8 py-3.5 h-[46px] flex items-center gap-2 shadow-[var(--shadow-lg)] disabled:opacity-50">
            <Archive size={16} /> Terminate & Archive
          </button>
        </div>
      </div>

      <div className="bg-white rounded-2xl border-2 border-[var(--danger)] shadow-sm overflow-hidden">
        <div className="bg-[var(--danger-soft)] p-5 border-b border-[var(--danger-soft)] flex items-center gap-4">
          <div className="w-10 h-10 rounded-full bg-white text-[var(--danger)] flex items-center justify-center shadow-sm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <button className="command-card p-8 flex flex-col items-center gap-4 bg-[var(--bg)] hover:bg-[var(--ink-100)] border border-[var(--border-strong)] hover:border-[var(--gc-purple)] transition-all group shadow-sm text-[var(--ink-400)] hover:text-[var(--gc-purple)]">
      ...
      <div className="command-card p-8 bg-[var(--bg)] border border-[var(--border)]">
        <div className="pb-4 border-b border-[var(--border)] mb-5">
           <h4 className="section-title text-[13px] tracking-widest">Post-Campaign Analysis (Lessons Learned)</h4>
        </div>
        <textarea 
          className="w-full min-h-[140px] px-5 py-4 border border-[var(--border-strong)] rounded-xl text-[14px] font-medium placeholder:text-[var(--ink-300)] bg-white focus:ring-[4px] focus:ring-[var(--gc-purple-soft)] focus:border-[var(--gc-purple)] outline-none transition-all shadow-sm"
        ...
        <div className="mt-4 flex justify-end">
          <button className="px-6 py-2.5 bg-white border border-[var(--border-strong)] rounded-xl text-[12px] font-bold text-[var(--ink-700)] hover:bg-[var(--ink-100)] transition-all shadow-sm">
            Save Draft
          </button>
        </div>
      </div>
